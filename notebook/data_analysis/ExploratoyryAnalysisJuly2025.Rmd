...
interim_beneficiarios_peps |> 
  filter(ultimo_registro_pep >as.Date("2024-01-01")) |> 
  filter(ultimo_registro_plan  >as.Date("2024-01-01")) |> 
  relocate(nombre_completo_pep,cargo,organismo, partido_pep, programa, distrito_pep, ultimo_registro_pep,ultimo_registro_plan, primer_registro_pep, primer_registro_plan)
```

```{r}
#MAGDA CILES LUCIANA AYALA 34544603
#Es legisladora provincial peronista por el chaco
#Fue intendenta de barranqueras enre 2019 y 2023
#Cobra el volver al trabajoy cobraba el potenciar

#LUCIANO GRACIANO MOSER
#Tmb legisladore del chaco
#De capitanich
#Viene cobrando AUH y alimentar hace rato
...
```{r}
#Nos concentramos en lo actual
interim_beneficiarios_empleos |> 
  filter(cobro_siendo_empleado) |> 
  filter(ultimo_registro_plan>as.Date("2024-01-01")) |> 
  filter(ultimo_registro_cargo >as.Date("2024-01-01")) |> 
  relocate(organismo, cargo, primer_registro_cargo, ultimo_registro_cargo, primer_registro_plan, ultimo_registro_plan)
